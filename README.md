...
The canonical text lives in **[agent-instructions.md](agent-instructions.md)** at the repo root — visible, reviewable, and copy-ready.

> [!IMPORTANT]
> GitHub enforces a **4,000 character limit** for organizational Custom Instructions. Keep [agent-instructions.md](agent-instructions.md) under that limit (CI validates on every PR).
...
````bash
curl -fsSL https://raw.githubusercontent.com/bcgov/agent-instructions/main/agent-instructions.md \
  -o .github/copilot-instructions.md
````

**Org-level:** configure custom agent instructions from the [raw file](https://raw.githubusercontent.com/bcgov/agent-instructions/main/agent-instructions.md) or paste its contents into org settings.
...
We want this to become something teams across BC can actually adopt. Open a PR to improve [agent-instructions.md](agent-instructions.md) — keep it under 4,000 characters, stay concrete, and explain the *why* in the PR description when a rule is non-obvious.
